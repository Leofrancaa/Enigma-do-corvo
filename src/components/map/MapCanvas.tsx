...
const PATH_STROKE   = "#24243a";
const ENTRY_BG      = "#0e0e1c";
const BOARD_BG      = "#06060f";
const BOARD_BORDER  = "#92400e";
const REACH_FILL    = "rgba(245,158,11,0.28)";
...
export function MapCanvas({ locations, players, reachable, onCellClick }: Props) {
  ...
  // Use ref for pan so click handler always sees latest value (no stale closure)
  ...
  function onPointerMove(e: React.PointerEvent<HTMLDivElement>) {
    ...
    applyPan(dragStart.current.px + dx, dragStart.current.py + dy);
  }
  ...
  // Click on container — calculate SVG cell with pan offset (works even with pointer capture)
  function onContainerClick(e: React.MouseEvent<HTMLDivElement>) {
    if (dragMoved.current) { dragMoved.current = false; return; }
    const container = containerRef.current;
    if (!container) return;
    const rect = container.getBoundingClientRect();
    // Subtract pan so we get SVG coordinates
    const svgX = e.clientX - rect.left - panRef.current.x;
    const svgY = e.clientY - rect.top  - panRef.current.y;
    ...
    >
      <svg
        ...
        style={{
          ...
          display: "block",
          touchAction: "none",
          pointerEvents: "none", // container handles all pointer/click events
        }}
      >
        <rect width={SVG_W} height={SVG_H} fill={BOARD_BG} />
        <rect x={2} y={2} width={SVG_W-4} height={SVG_H-4} fill="none" stroke={BOARD_BORDER} strokeWidth={4} rx={8} />

        {/* ── Room images (only within ROOM_RECTS, never over entry cells) ── */}
          ...
          return (
            <g key={slug}>
              ...
              <text
                x={x + w/2} y={y + h - 10}
                textAnchor="middle"
                fontSize={Math.min(16, Math.max(11, w / 6))}
        ...
        {players.map((p, idx) => {
          ...
          const offsets = [[0,0],[16,-10],[-16,-10],[0,16],[14,8],[-14,8]];
          const [ox, oy] = offsets[idx] ?? [0,0];
          const r = CELL_PX * 0.22;
          return (
            <g key={p.id} transform={`translate(${ox},${oy})`}>
              {p.isMe && (
                <circle cx={cx} cy={cy} r={r + 6} fill="none" stroke={color} strokeWidth={3}>
                  <animate attributeName="opacity" values="0.9;0.1;0.9" dur="2s" repeatCount="indefinite" />
                </circle>
              )}
              <circle cx={cx} cy={cy} r={r} fill={color} stroke={BOARD_BG} strokeWidth={3} />
              <text x={cx} y={cy + r*0.38} textAnchor="middle"
                fontSize={r * 0.9} fontFamily="monospace" fontWeight="bold" fill="#000">
